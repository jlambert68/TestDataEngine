...
func main() {
	...
	flag.StringVar(&testDataDomainUUID, "testdata-domain-uuid", "", "TestDataDomainUuid to store in data_items")
	flag.StringVar(&testDataDomainName, "testdata-domain-name", "", "TestDataDomainName to store in data_items")
	flag.StringVar(&sourceTemplateName, "testdata-source-template-name", "", "TestDataSourceTemplateName to store in data_items")
	...
	result, err := sqlitecsv.ImportRawCSV(context.Background(), sqlitecsv.ImportOptions{
		...
		TestDataSourceTemplateName: sourceTemplateName,
		TableName:                  tableName,
